...
PS: 在国内使用163账号测试发邮件没有问题，当把程序部署到东京服务器的时候，后台发送邮件时与163服务器连接超时，导致发信失败；此时需要考虑到国际环境，所以后面换成了Gmail账号，测试发信成功，如果是测试程序在国内的话，gmail发邮件是不行的，国内连接超时。所以邮件需要看最终部署的地方，国内的话，建议国内邮件服务；国外的话，建议Gmail邮件服务。Gmail在使用之前，需要在gmail安全设置中将less secure选项打开，不然会报身份验证失败。
