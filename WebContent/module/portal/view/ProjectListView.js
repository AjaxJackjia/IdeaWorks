define([ 
	...
	var ProjectItem = function(index, project) {
		...
		var _itemHeader_tpl = 
			...
			'	<h2 class="item-title"> ' +
			'		<a href="index.html#projects?id='+ project.get('projectid') + '">' + project.get('title') + '</a> ' + 
			'	</h2> ' + 
			'	<div class="item-time">' + project.get('time') + '</div> ' +
			'	<div class="line"></div> ' +
			'	<div class="item-description">' + project.get('content') + '</div> ' +
			'	<div class="item-action"> ' +
            '		<a href="index.html#projects?id='+ project.get('projectid') + '" class="read-more">' + i18n.portal.ProjectListView.READ_MORE + '<i class="fa fa-angle-right"></i></a> ' +
            '	</div> ' +
            '</div>';
